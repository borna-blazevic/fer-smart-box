...
void init_gpio_output()
{
    ...
    //configure GPIO with the given settingsd
}
...
void app_main(void)
{
    ...
    spi2_handle = malloc(sizeof(spi_device_handle_t));
    spi3_handle = malloc(sizeof(spi_device_handle_t));
    tera_spi2 = malloc(sizeof(uint32_t));
    tera_spi3 = malloc(sizeof(uint32_t));
    init_spi(tera_spi2, tera_spi3, spi2_handle, spi3_handle);
    while (1)
    {
        number = gpio_get_level(GPIO_INPUT_IO);
        if (number)
        {
            ...
            ESP_LOGI(TAG, "locked");
            message.type = DATA;
            message.number_data = mass2;
            strcpy(message.data[0], last_user);
            cToJSON(message, &json_message);
            strcpy(message.data[1], "load_cell1");
            ret = tls_write(json_message, conn);

            message.type = DATA;
            message.number_data = mass3;
            strcpy(message.data[0], last_user);
            cToJSON(message, &json_message);
